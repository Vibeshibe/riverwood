...
Implementation: static HTML, CSS, and JavaScript with no build step. The landing page lives in `index.html`; `about.html`, `privacy.html`, and `matrix-privacy.html` provide the information and legal pages, with their shared layout in `subpage.css`. Matrix discovery documents live under `.well-known/matrix`; the example nginx locations add the required JSON content types and CORS headers and preserve Mastodon WebFinger queries. Original SVG scenery remains editable and recolors with CSS variables. Typography uses local system fallbacks so opening the site does not contact a third-party font service. Service URLs are centralized in `script.js` and also supplied as HTML fallbacks. The site explains existing shared authentication; it does not implement the authentication backend.
